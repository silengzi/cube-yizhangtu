<template>
  <div id="map" class="map">
    <Popup ref="popup" :popupData="popupData"></Popup>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      overlay: null, //  弹窗
      popupData: null, // 弹窗数据
      heightLightSource: null, // 该变量用于专门加载高亮行政区划的数据
    };
  },
  mounted() {
    this.initMap();
  ...
  methods: {
    initMap() {
      ...
      const heightLigghtLayer = new VectorLayer({
        source: heightLightSource,
      });
      map.addLayer(heightLigghtLayer);
